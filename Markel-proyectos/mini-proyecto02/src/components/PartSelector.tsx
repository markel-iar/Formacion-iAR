        ...
        <div>
            ...
            <ul>
                {products.map((product) => (
                    <li key={product.id} style={{ backgroundColor: product.id ==  selectedId ? "#d3f9d8" : "transparent" }}>
                        <span>{product.nombre} - ${product.precio.toFixed(2)}</span>
                        <button 
                            onClick={() => onSelect(product)}
                            style={{ marginLeft: "10px", backgroundColor: product.id ===  selectedId ? "#4caf50" : "#007bff", color: "white" }}
                        >
